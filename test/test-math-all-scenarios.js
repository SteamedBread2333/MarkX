...
const allTests = [
    ...
    {
        name: '场景4：价格和数学公式在同一行',
        html: '<p>价格 $50，公式 $x = y + z$ 是数学公式。</p>',
        expectedPrices: 1,
        expectedFormulas: 1
    },
    {
        name: '场景5：标题和正文都有价格和公式',
        html: '<h2 id="标题价格-50">标题价格&nbsp;$50</h2><p>正文价格 $30，数学公式 $f(x) = x^2$。</p>',
        expectedPrices: 2,
        expectedFormulas: 1
    },
    {
...
allTests.forEach((test, index) => {
    ...
    if (!passed) {
        ...
        console.log(`  输入HTML: ${test.html}`);
        console.log(`  输出HTML: ${result.result}`);
        if (result.debug.pricePlaceholders.length > 0) {
            console.log(`  价格占位符:`, result.debug.pricePlaceholders.map(p => ({
                before: p.beforeChar,
                value: p.priceValue,
                after: p.afterChar
            })));
